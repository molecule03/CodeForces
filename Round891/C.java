...
//    Upsolved
    ...
    public static void main(String[] args) {
        ...
        for (int tt=0; tt<T; tt++) {
            ...
            for(int i=0; i<m; i++)arr[i] = fs.nextLong();

            Arrays.sort(arr);

            int in = 1;
            for(int i=(int)m-1; i>=0; i-= in){
                if(i>= m) break;
                out.print(arr[i]+" ");
                in++;
            }
            out.print((arr[(int)m-1]));
            out.println("");
        }
...
//        double discriminant = 1 + 8 * (long) m; // Avoid integer overflow by using long type for multiplication
