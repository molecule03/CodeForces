...
//        Scanner sc = new Scanner(System.in);
        ...
        for (int tt=0; tt<T; tt++) {
            ...
            long n = fs.nextLong();
            long totVer = (long) Math.sqrt(n);
            boolean flag = true;
            for (long i = resVal+1; i <= totVer; i++) {
                long val = resVal;
                while (Math.pow(i, val - resVal) <= n)  val++;

                for (long j = resVal; j <= val + resVal; j++) {
                    long k = (long) ((Math.pow(i, j) - resVal) / (i - resVal));
                    if (k == n) {
                        flag = false;
                        break;
                    }
